...
if ( ! class_exists( 'Blogone_Walker_Page' ) ) {
	/**
	 * CUSTOM PAGE WALKER
	 * A custom walker for pages.
	 */
	class Blogone_Walker_Page extends Walker_Page {
		...
		public function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
			...
			if ( '' === $page->post_title ) {
				...
				$page->post_title = sprintf( __( '#%d (no title)', 'blogone' ), $page->ID );
			}
			...
			if ( isset( $args['show_toggles'] ) && true === $args['show_toggles'] ) {
				...
				if ( isset( $args['pages_with_children'][ $page->ID ] ) ) {
					...
					$toggle_duration      = blogone_toggle_duration();

					// Add the sub menu toggle.
					$args['list_item_after'] .= '<button type="button" class="toggle-menu"><i class="fa fa-angle-down"></i></button>';
