...
if( ! function_exists('blogone_default_options') ){
	function blogone_default_options(){
		$data = array(
            'blogone_mode' => 'light', // dark
            'blogone_container_full' => false,
            'blogone_h_logo_width' => 150,
            'blogone_h_site_title_fontsize' => 36,
            'blogone_h_site_desc_fontsize' => 14,
            'blogone_h_sticky_show' => true,
            'blogone_accent_color' => '#ffb14f',
            'blogone_text_color' => '#4b505a',
            'blogone_heading_color' => '#302d55',
            'blogone_layout' => 'wide',

            'blogone_breadcrumb_show' => true,
            'blogone_breadcrumb_title_show' => true,
            'blogone_breadcrumb_path_show' => true,
            'blogone_breadcrumb_bg_color' => '',
            'blogone_breadcrumb_bg_image' => '',
            'blogone_breadcrumb_attachment' => 'scroll',
            'blogone_breadcrumb_repeat' => 'no-repeat',
            'blogone_breadcrumb_position' => 'center',
            'blogone_breadcrumb_size' => 'cover',
            'blogone_breadcrumb_overlay' => '',
            'blogone_breadcrumb_height' => '100',

            'blogone_archive_excerpt_length' => 50,
            'blogone_archive_readmore_label' => __('Read More','blogone'),
            'blogone_archive_category_show' => true,
            'blogone_archive_title_show' => true,
            'blogone_archive_date_show' => true,
            'blogone_archive_excerpt_show' => true,
            'blogone_archive_btn_show' => true,
            'blogone_archive_author_show' => true,

            'blogone_single_category_show' => true,
            'blogone_single_title_show' => true,
            'blogone_single_date_show' => true,
            'blogone_single_author_show' => true,

            'blogone_page_title_show' => true,
            'blogone_page_author_show' => true,

            'blogone_blog_show' => true,
            'blogone_blog_masonary' => false,
            'blogone_blog_sidebar' => '0-1-1',
            'blogone_blog_category' => '',
            'blogone_blog_exclude_ids' => '',
            'blogone_blog_orderby' => 'date',
            'blogone_blog_order' => 'desc',
            'blogone_blog_posts_per_page' => 5,
            'blogone_blog_readmore' => __('Read More','blogone'),
            'blogone_blog_category_show' => true,
            'blogone_blog_title_show' => true,
            'blogone_blog_date_show' => true,
            'blogone_blog_excerpt_show' => true,
            'blogone_blog_btn_show' => true,
            'blogone_blog_author_show' => true,
            'blogone_blog_column' => 1,
            
            'blogone_footer_widget_column' => 4,
            'blogone_footer_copyright_show' => true,
            'blogone_footer_copyright' => 'Copyright %copy% %current_year%, All Rights Reserved.',            
            'blogone_footer_bg_color' => '',
            'blogone_footer_bg_image' => '',
            'blogone_footer_bg_attachment' => 'scroll',
            'blogone_footer_bg_repeat' => 'no-repeat',
            'blogone_footer_bg_position' => 'center',
            'blogone_footer_bg_size' => 'cover',
            'blogone_footer_overlay' => '',
            'blogone_footer_backtotop' => true,

            'blogone_error_code' => __('4<span><i class="far fa-frown"></i></span>4','blogone'),
            'blogone_error_subtitle' => __('Error','blogone'),
            'blogone_error_title' => __('Page Not Found','blogone'),
            'blogone_error_desc' => __('It looks like nothing was found at this location. Maybe try one of the links below or a search?','blogone'),
            'blogone_error_btn_label' => __('Back To Home','blogone'),
		);

		$data = apply_filters('blogone_default_options',$data);
		return $data;
	}
}

if( ! function_exists('blogone_theme_options') ){
    function blogone_theme_options(){
        $options = blogone_default_options();
        $data = array();
        foreach($options as $key => $val){
            $data[$key] = get_theme_mod($key,$options[$key]);
        }
        return $data;
    }
}

function blogone_change_theme_options( $key, $value ){
    $options = blogone_theme_options();
    if( isset( $options[$key] ) ){
        $options[$key] = $value;
    }
}

function blogone_categories(){
...
function blogone_tags(){
    ...
    foreach( $tags as $tag ) {
        $tag_arg[$tag->term_id] =  $tag->name .' ('.$tag->count.')';
    }
    return $tag_arg;
}

function blogone_blendChannels(float $alpha, int $channel1, int $channel2): int{
    // blend 2 channels
    return intval(($channel1 * $alpha) + ($channel2 * (1.0 - $alpha)));
}

function blogone_convertRGBAtoHEX6(string $rgba): string{
    // sanitize
    ...
    $alpha = $channels[3];
    $r = blogone_blendChannels($alpha, $channels[0], 0xFF);
    $g = blogone_blendChannels($alpha, $channels[1], 0xFF);
    $b = blogone_blendChannels($alpha, $channels[2], 0xFF);
    return sprintf('#%02x%02x%02x', $r, $g, $b);
}

function blogone_fontawesome_icons_list(){
	return array (
        'fab fa-500px' => '500px',
