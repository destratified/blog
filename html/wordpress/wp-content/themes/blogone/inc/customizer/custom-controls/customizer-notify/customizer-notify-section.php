<?php
class Blogone_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'blogone-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'blogone_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'blogone_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		$blogone_customizer_notify_show_recommended_actions = isset($option['blogone_customizer_notify_show'])?$option['blogone_customizer_notify_show']:'';
		foreach ( $blogone_customizer_notify_recommended_actions as $key => $blogone_customizer_notify_recommended_action ) {
			if ( $blogone_customizer_notify_show_recommended_actions[ $blogone_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $blogone_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$blogone_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $blogone_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $blogone_customizer_notify_recommended_action['plugin_slug'] );
				$blogone_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $blogone_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$blogone_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$blogone_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$blogone_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$blogone_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$blogone_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$blogone_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$blogone_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$blogone_customizer_notify_recommended_action['button_label'] = $deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $blogone_customizer_notify_recommended_action;
		}// End foreach().

		$customize_plugins = array();

		$blogone_customizer_notify_show_recommended_plugins = isset($option['blogone_customizer_notify_show_recommended_plugins'])?$option['blogone_customizer_notify_show_recommended_plugins']:'';

		foreach ( $blogone_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $blogone_customizer_notify_show_recommended_plugins[ $slug ] ) && $blogone_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
		...
		$json['total_actions']       = count( $blogone_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="blogone-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="blogone-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="blogone-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="blogone-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no blogone-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="blogone-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="blogone-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no blogone-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
